...
import org.irisacsee.trajlab.index.type.TimeElement;
...
public class XZTSFC implements Serializable {
    ...
    public TimeBin getTimeBin(long xztValue) {
        int bid = (int) (xztValue / binElementCnt());
        return new TimeBin(bid, timePeriod);
    }
    ...
    public long index(TimeLine timeLine, TimeBin timeBin) {
        return index(timeLine.getTimeStart(), timeLine.getTimeEnd(), timeBin);
    }

    public long index(ZonedDateTime start, ZonedDateTime end, TimeBin timeBin) {
        double[] binTime = normalize(start, end, timeBin, true);
        double length = binTime[1] - binTime[0];
        int l1 = (int) Math.floor(Math.log(length) / CodeConstant.LOG_FIVE);
        int level;
        if (l1 >= g) {
            level = g;
        } else {
        ...
        long binBase = (long) binElementCnt() * timeBin.getBid();
     ...
     * @return 更新后的ranges.
     */
    public List<SFCRange> containSequenceCodeRange(
            TimeElement timeElement,
            TimeLine timeQuery,
            short level,
            List<SFCRange> ranges,
            Deque<TimeElement> remaining,
            TimeBin timeBin) {
        if (timeElement.isExContainedBy(timeQuery)) {
            SFCRange timeIndexRange =
                    sequenceInterval(timeElement.getTimeStart(), level, timeBin, false, true);
            ...
            if (canStoreContainedObjects(timeElement, timeQuery)) {
                SFCRange timeIndexRange =
                        sequenceInterval(timeElement.getTimeStart(), level, timeBin, true, false);
                ranges.add(timeIndexRange);
            }
     ...
     * @return 更新后的ranges.
     */
    public List<SFCRange> overlapSequenceCodeRange(
            TimeElement timeElement,
            TimeLine timeQuery,
            short level,
            List<SFCRange> ranges,
        ...
        SFCRange timeIndexRange;
        if (timeElement.isExContainedBy(timeQuery)) {
            timeIndexRange = sequenceInterval(timeElement.getTimeStart(), level, timeBin, false, true);
            ranges.add(timeIndexRange);
        } else if (timeElement.isExOverlaps(timeQuery)) {
            // 若timeQuery包含了timeElement右端点，则timeElement内的时间范围必与timeQuery相交。
            if (timeQuery.getReTimeStart() <= timeElement.getTimeEnd()
                    && timeQuery.getReTimeEnd() >= timeElement.getTimeEnd()) {
                timeIndexRange = sequenceInterval(timeElement.getTimeStart(), level, timeBin, true, true);
            } else {
                timeIndexRange = sequenceInterval(timeElement.getTimeStart(), level, timeBin, true, false);
            }
            ranges.add(timeIndexRange);
            remaining.addAll(timeElement.getChildren());
        }
    ...
    public List<SFCRange> getMaxLevelSequenceCodeRange(
            short level,
            TimeLine timeQuery,
            List<SFCRange> ranges,
            Deque<TimeElement> remaining,
        ...
        while (!remaining.isEmpty()) {
            ...
            if (poll.equals(levelSeparator)) {
                ...
                if (poll.isExContainedBy(timeQuery)) {
                    SFCRange timeIndexRange =
                            sequenceInterval(poll.getTimeStart(), level, timeBin, false, true);
                    ranges.add(timeIndexRange);
                } else if (poll.isExOverlaps(timeQuery)) {
                    SFCRange timeIndexRange =
                            sequenceInterval(poll.getTimeStart(), level, timeBin, false, false);
                    ranges.add(timeIndexRange);
                }
            }
        }
        ...
        while (i < ranges.size()) {
            ...
            if (indexRange.getLower() == current.getUpper() + 1
                ...
                current =
                        new SFCRange(
                                current.getLower(),
                                Math.max(current.getUpper(), indexRange.getUpper()),
                                indexRange.isValidated());
            } else {
     ...
     * @return
     */
    public SFCRange sequenceInterval(
            double timeStart,
            short level,
            TimeBin timeBin,
            Boolean singleRange,
        ...
        return new SFCRange(getXZTValue(timeBin, min), getXZTValue(timeBin, max), isValidateRange);
    }

    public long getXZTValue(TimeBin timeBin, long elementCode) {
        return (long) timeBin.getBid() * binElementCnt() + elementCode;
    }
    ...
    /**
     * 检查TimeElement是否有可能管理了完全包含于timeQuery的时间范围。 依据： 当element层级l < g.
     * <li><item>timeQuery应跨element扩展单元的两个半区</item> <item>timeQuery应穿过两条以上l+1层的时段分割线</item>
     *
     * @param element
     * @param timeQuery
     * @return
    ...
    private boolean canStoreContainedObjects(TimeElement element, TimeLine timeQuery) {
        if (element.isContainedBy(timeQuery) || element.isOverlaps(timeQuery)) {
            ...
            // 比end小的最大整数
            int upperLineID =
                    (int) (extOverlappedTimeLine.getReTimeEnd() / finnerElementLen)
                            - extOverlappedTimeLine.getReTimeEnd() % finnerElementLen
                            == 0
                            ? 1
                            : 0;
    ...
    /**
     * 以timeBin为参照，将时间范围归一化。 对于轨迹的时间范围，应保证其结束时间小于ext bin所覆盖的最大时间。 对于查询的时间范围，其时间范围没有限制。
     *
    ...
    public double[] normalize(
            ZonedDateTime startTime, ZonedDateTime endTime, TimeBin timeBin, Boolean extBinContained) {
        double nStart =
                (timeBin.getRefTime(startTime) * 1.0)
        ...
        if (extBinContained) {
            if (timePeriod
                    .getChronoUnit()
                    .getDuration()
                    .compareTo(Duration.ofSeconds(timeBin.getRefTime(endTime) / 2))
                    > 0) {
                nEnd =
                        timeBin.getRefTime(endTime)
                                * 1.0
                                / timePeriod.getChronoUnit().getDuration().getSeconds();
            } else {
                log.error(
                ...
                System.out.println("tp: " + timePeriod.getChronoUnit().getDuration());
                System.out.println("comp: " + Duration.ofSeconds(timeBin.getRefTime(endTime) / 2));
                System.out.println("=================");
