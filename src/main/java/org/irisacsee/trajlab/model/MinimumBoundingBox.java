...
import org.irisacsee.trajlab.util.CheckUtil;
...
public class MinimumBoundingBox extends Envelope {
    ...
    private void calcGeoLength() {
        ...
        this.latGeoLength = GeoUtil.getEuclideanDistanceKM(this.getLowerLeft(), var1);
        var1 = new BasePoint(this.getMaxLng(), this.getMinLat());
        this.lngGeoLength = GeoUtil.getEuclideanDistanceKM(this.getLowerLeft(), var1);
    }
    ...
    private static double getMBR2PointDis(MinimumBoundingBox mbr, BasePoint p) {
        ...
        if (mbr.isContains(p)) {
            ...
            return GeoUtil.getEuclideanDistanceKM(var7, p);
        }
    }
    ...
    public static MinimumBoundingBox calMinimumBoundingBox(Collection plist, Function func) {
        if (CheckUtil.isCollectionEmpty(plist)) {
            return null;
        } else {
