...
//        }
        ...
        for (String dataSetName : DATA_SET_NAMES) {
            for (String mode : MODES) {
                for (int runIndex : RUNS_INDEX) {
                    for (int kIndex : KS_INDEX) {
                        for (int timeIndex : TIME_LINES_INDEX) {
                            testKnn(TIME_LINES[timeIndex], dataSetName, mode, RUNS[runIndex], KS[kIndex]);
                            System.gc();
    ...
    private static void testKnn(TimeLine timeLine, String dataSetName, String mode, String run, int k) throws IOException {
        HBaseConnector.getConnection();
        long start = System.currentTimeMillis();
        TemporalQueryCondition tqc = new TemporalQueryCondition(timeLine,
...
//        KNNQueryCondition knnQueryCondition = new KNNQueryCondition(k, CEN_TRAJECTORY, tqc);
        DataSetMeta dataSetMeta = HBaseConnector.getDataSetMeta(dataSetName);
...
//        }
        ...
        System.out.println(
                "dsn: " + dataSetName + ", mode: " + mode + ", run: " + run + ", k: " + k + ", tl: " + timeLine);
        Trajectory[] results = null;
...
//            IOUtil.writeStringToFile("/home/oge/ods/data/1.json", jsonObject.toString());
    ...
    private static void testKnnQuery(int k) throws IOException {
        long start = System.currentTimeMillis();
...
//        KNNQueryCondition knnQueryCondition = new KNNQueryCondition(k, CEN_TRAJECTORY, tqc);
        ...
        KNNQuery knnQuery = new KNNQuery(
                HBaseConnector.getDataSetMeta("T100TS"), knnQueryCondition);
...
//                "C:\\Users\\22772\\Desktop\\模拟面试\\p10.json", jsonObject.toString());
    }
...
//        KNNQueryCondition knnQueryCondition = new KNNQueryCondition(k, CEN_TRAJECTORY, tqc);
        ...
        try (SparkSession sparkSession =
                     SparkUtil.createSession(KNNQueryProcess.class.getName())) {
            List<Trajectory> results = knnQuery.runBySpark(sparkSession);
            System.out.println("size: " + results.size());
//            for (Trajectory result : results) {
//                System.out.println(result);
